...
It performs an example run from the `Beyond-2pt <https://github.com/ANSalcedo/Beyond2ptMock>`_ blind data challenge, based on a simulated light-cone in a \ *w*\ CDM cosmology (`C_mock_lightcone.h5 <https://github.com/ANSalcedo/Beyond2ptMock/blob/main/C_mock_lightcone.h5>`_). The example data is stored in the `catalogs/ <https://github.com/nhamaus/Voiager/tree/main/catalogs/>`_ folder including void catalogs produced with `VIDE <https://bitbucket.org/cosmicvoids/vide_public/wiki/Home/>`_, while the output of the code can be found in the `results/ <https://github.com/nhamaus/Voiager/tree/main/results/>`_ directory.

.. note::
    The example data in `catalogs/ <https://github.com/nhamaus/Voiager/tree/main/catalogs/>`_ and the example results in `results/ <https://github.com/nhamaus/Voiager/tree/main/results/>`_ are not automatically downloaded upon installation, but they can be accessed via the repository.
